...
1. 在你想存放项目的位置（比如桌面），**右键** → 选择 **"在终端中打开"**（Windows 11）或 **"Git Bash Here"**

   > 如果你没有看到这些选项，可以按 `Win + R`，输入 `cmd`，回车，然后手动进入目标目录：
   > ```
...
**停止服务：** 在终端按 `Ctrl + C`，输入 `Y` 确认。

---

## 常见问题

**Q: 终端提示 `git` / `node` / `npm` 不是内部或外部命令？**
A: 安装 Git 或 Node.js 后需要**重启电脑**，让系统识别新安装的命令。

**Q: `npm install` 卡住或报错？**
A: 大概率是网络问题，运行以下命令切换国内镜像后重试：
```
npm config set registry https://registry.npmmirror.com
npm install
```
...
**Q: AI 功能不工作？**
A: 在设置页面检查 API Key、Base URL 和模型名称是否正确填写，并确保点击了"保存设置"。
