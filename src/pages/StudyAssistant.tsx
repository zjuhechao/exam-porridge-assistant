...
import { getDocumentsByCourse, getSummariesByCourse, getNotesByCourse, createSummary, createNote, getCurrentCourseId } from '../services/db';
import { generateSummary, generateNotes, getAPIConfigForFunction } from '../services/api';
...
export function StudyAssistant() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      // 合并多个文档内容
      const combinedContent = selectedDocuments.map(d => `## ${d.filename}\n\n${d.content || ''}`).join('\n\n---\n\n');
      const combinedTitle = selectedDocuments.length === 1 ? selectedDocuments[0].filename : `${selectedDocuments.length}个文档合集`;

      if (activeTab === 'summary') {
        const result = await generateSummary(combinedContent, combinedTitle, { apiConfig });
        setGeneratedContent(result.summary);
        await createSummary({
          document_id: selectedDocuments[0].id,
